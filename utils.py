...
def get_test_id(num):
    im_id = []
    test_id_path = "/data/xiaobing.wang/qy.feng/data/coco_pytorch/test_img_id.txt"
    with open(test_id_path, 'r') as f:
        test_id = f.readlines()
    for id in test_id[:num]:
        im_id.append(int(id.split('\n')[0]))  
        
    im_list = []
    test_list_path = "/data/xiaobing.wang/qy.feng/data/coco_pytorch/test_img_list.txt"
    with open(test_list_path, 'r') as f:
        test_list = f.readlines()
    for path in test_list[:num]:
        im_list.append(path.split('\n')[0])
    
    return im_id, im_list
    

def show_pose(imgs, heats, vecs, masks, num):
    ...
    for i in range(num):
        ...
        heat = heat.transpose(1, 2, 0)# 46, 46, 19
        heatmap_avg = cv2.resize(heat, (h, w))
        
        vec = vec.transpose(1, 2, 0) # 46, 46, 38
        paf_avg = cv2.resize(vec, (h, w))
        ...
        for n in range(len(subset)):
            ...
            for i in range(0,18):
                ...
                person.append([ID,X,Y,score])
            final_score = subset[n][-1]
        
            #print 'final_score', final_score
            result.append([person, final_score])
    ...
    return mask_res, heat_res, vec_res


def show_ori_pose(img, heat, vec, mask):
    
    _, h, w = img.shape# 3, 368, 368
    ...
    for i in range(18):
        for j in range(len(all_peaks[i])):
            c = [co/128 for co in colors[i]]
            cv2.circle(canvas, all_peaks[i][j][0:2], 4, c, thickness=-1)
    h = canvas
    ...
    for i in range(17):
        for n in range(len(subset)):
            ...
            polygon = cv2.ellipse2Poly((int(mY), int(mX)), (int(length / 2), stickwidth), int(angle), 0, 360, 1)
            c = [co/128 for co in colors[i]]
            cv2.fillConvexPoly(cur_canvas, polygon, c)
            canvas = cv2.addWeighted(canvas, 0.4, cur_canvas, 0.6, 0)
    v = canvas
