...
class resnet50_conv3(nn.Module):

    def __init__(self, net_dict, batch_norm=False):

        super(resnet50_conv3, self).__init__()
        ...
        self.conv_reduce = nn.Conv2d(512, 128, 1, 1, 0)
...
def PoseModel(num_point, num_vector, num_stages=6, batch_norm=False, pretrained=False):
    ...
    model = resnet50_conv3(net_dict, batch_norm)
    pretrain_model_file = '/root/.torch/models/resnet50-19c8e357.pth'
    print("Loading pretrained weights from %s" % (pretrain_model_file))
    model_dict = model.state_dict()
    pretrain_state_dict = torch.load(pretrain_model_file)
    pretrain_state_dict = {k: v for k, v in pretrain_state_dict.items() if k in model_dict}
