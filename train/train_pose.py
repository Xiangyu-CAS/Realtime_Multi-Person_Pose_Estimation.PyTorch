...
def parse():
    ...
    parser.add_argument('--val_dir', nargs='+', type=str,
                        ...
                        default=['/home/xiangyu/data/samsung_pose_data/img_list/minival2014.txt',
                                 '/home/xiangyu/data/samsung_pose_data/mask_list/minival2014.txt',
                                 '/home/xiangyu/data/samsung_pose_data/json/minival2014.json'])
...
def train_val(model, args):
    ...
    while iters < config.max_iter:
        ...
        for i, (input, heatmap, vecmap, mask, kpt) in enumerate(train_loader):
            ...
            loss_list = [loss1_1, loss1_2, loss2_1, loss2_2, loss3_1, loss3_2, loss4_1, loss4_2, loss5_1, loss5_2, loss6_1, loss6_2]
    ...
    #------------------------------------------ val ---------------------------------------------------------------------
            ...
            #     model.eval()
            #     for j, (input, heatmap, vecmap, mask, kpts) in enumerate(val_loader):
            #         imgs = input.numpy()
            ...
            #             all_peaks, subset, candidate = vis_util.get_pose(heatmap[i], vecmap[i])
            #             canvas_targ = img.copy()
            #             vis_util.draw_pose(canvas_targ, all_peaks, subset, candidate)
            ...
            #             img = img.transpose(1, 2, 0)  # 368, 368, 3
            #             img = (img + 1) / 2 * 255
            #             all_peaks, subset, candidate = vis_util.get_pose(heat_out[i], vec_out[i])
            #             canvas_pred = img.copy()
            #             vis_util.draw_pose(canvas_pred, all_peaks, subset, candidate)
