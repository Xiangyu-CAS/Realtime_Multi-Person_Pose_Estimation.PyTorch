...
def crop(img, mask, kpt, center, offset_left, offset_up, w, h):
    ...
    '''print new_img.shape
    print st_y, ed_y, st_x, ed_x
    print img.shape
    print or_st_y, or_ed_y, or_st_x, or_ed_x
    
    st_y, ed_y, st_x, ed_x = min(st_y, ed_y), max(st_y, ed_y), min(st_x, ed_x), max(st_x, ed_x)
    or_st_y, or_ed_y, or_st_x, or_ed_x = min(or_st_y, or_ed_y), max(or_st_y, or_ed_y), min(or_st_x, or_ed_x), max(or_st_x, or_ed_x)'''
...
def hflip(img, mask, kpt, center):
    ...
    for x in swap_pair:
        for i in range(num):
            temp_point = kpt[i][x[0] - 1]#.copy()
            kpt[i][x[0] - 1] = kpt[i][x[1] - 1]#.copy()
            kpt[i][x[1] - 1] = temp_point#.copy()
